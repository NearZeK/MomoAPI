...
class Momo
{
    ...
    private $URLAction = array(
        ...
        "RESET_PIN" => "https://api.momo.vn/backend/auth-app/public/RESET_PIN", // Đăng Nhập
        "RE_LOGIN" => "https://api.momo.vn/backend/otp-app/public/RE_LOGIN", // Đăng Nhập
        ...
        "TRANS_ID_HISTORY"                     => "https://api.momo.vn/sync/transhis/details", //check nội dung giao dịch
    );
    ...
    public function CheckHistoryV2($day, $limit)
    {
        ...
        foreach ($HisList as $transaction) {
            $result = $this->CheckHistCmt($transaction['transId']);

            if (empty($result["momoMsg"]['transId']) or empty($result["momoMsg"]['io'])) continue;
            $tranList[] = array(
                'ID'    => $result["momoMsg"]['id'],
                "tranId" => $result["momoMsg"]["transId"],
                "io"    => $result["momoMsg"]["io"],
                "partnerId" => $result["momoMsg"]["sourceId"],
                "status" => $result["momoMsg"]["status"],
                "partnerName" => empty($result["momoMsg"]["sourceName"]) ? "" : $result["momoMsg"]['sourceName'],
                "amount" => empty($result["momoMsg"]["totalAmount"]) ? 0 : $result["momoMsg"]["totalAmount"],
                "comment" => json_decode($result['momoMsg']['serviceData'], true)['COMMENT_VALUE'],
                "millisecond" => empty($result["momoMsg"]["lastUpdate"]) ? 0 : $result["momoMsg"]['lastUpdate']
            );
        }
        ...
        return json_encode(array(
            "status" => true,
            'message' => 'Thành công',
            "momoMsg" => array("tranList" => $tranList)
        ), JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
    }
    public function CheckHistCmt($transid)
    {
        ...
        return $this->CURL("TRANS_ID_HISTORY", $header, $this->Encrypt_data($Data, $requestkeyRaw));
    }
    ...
    public function SendMoneyBank($bankCode, $BankId, $amount = 10000, $comment = '')
    {
        if (empty($bankCode)) {
            return array(
                'status'  => 'error',
    ...
    public function checkNameBank($bankCode, $BankId)
    {
        ...
        return array(
            "status" => '2',
            "code" => 'Thành công',
            "message" => $result['benfAccount']['accName']
        );
    }
    private function TRAN_HIS_INIT_MSG_2()
    {
        ...
        $Data = array(
            ...
            array(
                ...
                'extras' => '{"saveCard":true,"bankNumber":"' . $this->send['accId'] . '","bankName":"' . $this->send['bankName'] . '","benfPhoneNumberDetect":"","vpc_CardType":"SML","vpc_TicketNo":"116.104.162.112","vpc_PaymentGateway":""}',
                'moneySource' => 1,
                'partnerCode' => 'momo',
                'rowCardId' => '',
                'giftId' => '',
    ...
    public function RE_LOGIN()
    {
        ...
        $header = array(
            ...
            'user_phone' => '',
            'app_code' => $this->momo_data_config["appCode"],
            'vsign' => '',
            'User-Agent' => 'okhttp/3.14.17',
            'app_version' => $this->momo_data_config["appVer"],
            'lang' => 'vi',
            'sessionKey' => '',
            'channel' => 'APP',
            'momo-session-key-tracking' => $this->config['imei'],
        ...
        return $this->CURL("RE_LOGIN", $header, $Data);
    }
